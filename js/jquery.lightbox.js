...
(function ($) {
    ...
    $.fn.lightbox = function (options) {
        ...
        var plugin = {
            ...
            init: function (items) {
                ...
                if (!plugin.lightbox) {
                    $('body').append(
                      ...
                      '<a href="#" class="lightbox-download lightbox-button2"></a>' +
                      ...
                      '<img src="images/gallery/1.jpeg" alt="Fuck">' +
                      '</div>'
                    );
            ...
            res2: function () {
                var factor, ratio, wHeight, wWidth, iHeight, iWidth;
                wHeight = $(window).height() - opts.margin;
                wWidth = $(window).outerWidth(true) - opts.margin;
                plugin.image.width('').height('');
                iHeight = plugin.image.height();
                iWidth = plugin.image.width();
                factor = 3
                if (iWidth > (wWidth / factor)) {
                    ratio = wWidth / (iWidth * factor);
                    iWidth = wWidth / factor;
                    iHeight = Math.round(iHeight * ratio);
                }
                if (iHeight > (wHeight / factor)) {
                    ratio = wHeight / (iHeight * factor);
                    iHeight = wHeight / factor;
                    iWidth = Math.round(iWidth * ratio);
                }
            ...
            bindEvents: function () {
                ...
                // Doownload click
                $(plugin.lightbox).on('click', '.lightbox-download', function () {
                    plugin.res2();
                    return false;
                });
